...
import AjoutRL from "../../components/responsable-livraison/ajout-livreur";
import ModifierLivreur from "../../components/responsable-livraison/modifier-livreur";
  ...
  useEffect(() => {
    const sendRequest = async () => {
      try {
        ...
        setlist(responseData.existingLivreur);
      } catch (err) {
    ...
    <Container>
      <Row>
        ...
        <Col xs={12}>
          ...
          <AjoutRL show={modalShow} onHide={() => setModalShow(false)} />
                      ...
                      <StyledTableRow key={row.name}>
                        ...
                        <StyledTableCell align="right">
                          <UpdateIcon
                            style={{ color: "green" }}
                            onClick={() =>
                              openModelUpdate(row._id, row.name, row.email)
                            }
                          />
                            ...
                            onClick={async (event) => {
                              try {
                                let response = await fetch(
                                  `http://localhost:5000/api/livreur/${row._id}`,
                                  {
                                    method: "DELETE",
                                    headers: {
                                ...
                                setsuccess("Livreur bien suprimer");
                              } catch (err) {
                                console.log(err);
